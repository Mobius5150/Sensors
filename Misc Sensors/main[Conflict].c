...
{
    long int result;
    unsigned long int corr;
    char DATAH = 0x00; char DATAL = 0x00;
    int j;
    ...
    while (1)
    {
        ...
        if(need_to_ack == 1)
        {
            ...
            corr = (result - 1660)*466/1024;
            sensorData[0][2] = 30 + 22.6 + corr/10;
            
            // Acquire the trunk temperature:
            ADCON0bits.ADON = 0;   // Disable A/D module
            ...
            // Backup alarm reading (0 to 1023):
            DATAH = (result>>8);
            DATAL = ((result<<8)>>8);
            ...
            sensorData[2][1] = 0;
            sensorData[2][2] = 120;//30 + 22.6 + corr/10;
